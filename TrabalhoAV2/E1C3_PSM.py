...
class MLP:
    ...
    def __init_weights__(self, p):
        ...
        self.W.append(
            np.random.random_sample((self.m, self.Q[-1] + 1)) - 0.5
        )  # Camada de saída
    ...
    def forward(self, x_sample):
        ...
        for j in range(self.L + 1):
            if j == 0:
                ...
                y_prev_bias = np.insert(
                    self.y[j - 1], 0, -1
                )  # y_prev_bias = [-1, y_prev]
    ...
    def backward(self, x_sample, d):
        ...
        last_layer = (
            len(self.W) - 1
        )  # len(self.W) = camada oculta (L) + camada de saída(1) = self.L + 1
        ...
        for j in range(last_layer, -1, -1):
            if j + 1 == len(self.W):  # Camada de saída
                ...
                self.W[j] += self.lr * np.outer(
                    self.δ[j], y_prev_bias
                ...
                self.δ[j] = self.__sigmoid_derivative__(self.y[j]) * np.dot(
                    W_b.T, self.δ[j + 1]
                )
    ...
    def __calc_eqm__(self, X, Y):
        ...
        for k in range(N):
            ...
            for n in range(
                self.m
            ):  # n = quantidade de neurônios na camada de saída — Basicamente a quantidade de classes
                EQI += (d_k[n] - y_k[n]) ** 2
    ...
    def predict(self, X, activation_function: bool = True):
        ...
        for k in range(N):
            ...
            if activation_function:
                print(self.y[-1])
                Y[:, k] = self.activation_function(
                    self.y[-1]
                )  # Saída da camada de saída (m)
            else:
